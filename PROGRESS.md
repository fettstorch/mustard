...
- Reposts: a repost is a **visibility grant** (not a new note/quote) — your followers can see a note even without following its original author; `reposts` table (migration 006: `note_id`→`notes` cascade, `reposter_id` DID, `UNIQUE(note_id, reposter_id)`, public read + reposter-only insert/delete RLS). Resolved **alongside** (not merged into) the author index to avoid leaking the author's other notes: `get-index-v2` queries reposts by `reposter_id IN [me + my follows]` and returns `repostedNoteIds` + `repostersByNoteId`; `MustardNotesServiceRemote.queryNotes` runs a second by-id fetch for reposted notes and dedupes, attaching network-scoped `reposterIds` to each `MustardNote` (model + DTO extended). Mutation via `SET_REPOST` message → `MustardNotesManager.setRepost` (insert/delete, idempotent upsert) → invalidate index cache → re-query (same re-query pattern as delete/comments). UI: `RepostAvatarStack` renders original author front-most with up to 3 reposter avatars peeking behind (`+N` overflow chip); repost `IconButton` fades in on note-hover only, 360° ease-out spin on press, disabled on own notes
